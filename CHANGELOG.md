...
- Add the optional `tmux-workspace` executable for workspace discovery, loading,
  capture, conversion and imports, with JSON and NDJSON output. (#17)
...
- `--log-file` records a warning at the `warning` level, so `--log-level
  warning` keeps the warnings it names. `--json` and `--ndjson` are read
  from the parsed command line, so a value like `load -s --json w.yaml`
  creates a session named `--json` and prints human output; the same holds
  for every other value-taking option. (#17)
...
- Accept v2 JSON saved layouts on tmux 3.9, including `next-3.9`, while
...
- `Client` rows carry `pid`, `active_pane_id`, `flags` and `window_id`, so a
  ...
  from the same snapshot as its terminal and session. (#17)
