...
def port_version(port: pathlib.Path) -> tuple[str, str, int] | None:
    """Return the version triple ``port`` declares, or None if it declares none."""
    manifest = port / "vcpkg.json"
    if not manifest.is_file():
        return None
    return declared_version(json.loads(manifest.read_text()))


def check_port(root: pathlib.Path, port: pathlib.Path) -> list[str]:
    ...
    field, version, revision = declared
    ...
    if not versions_path.is_file():
        return [
            (
                ...
                f"`python3 -m tools.vcpkg check --how` for the command that "
                f"writes it."
            ),
        ]
    ...
    matching = [
        ...
        if entry.get(field) == version and entry.get("port-version", 0) == revision
    ]
    if not matching:
        published = (
            ", ".join(
        ...
        problems.append(
            ...
            f"{version}#{revision}, which ports/{name}/vcpkg.json declares. "
            f"It publishes {published}.",
        )
    elif len(matching) > 1:
        problems.append(
            f"{versions_path.relative_to(root)}: {len(matching)} entries for "
            f"{version}#{revision}; a version is published once.",
        )
    else:
        ...
        if not actual:
            ...
            problems.append(
                f"{versions_path.relative_to(root)}: {version}#{revision} "
    ...
    elif entry.get("baseline") != version or entry.get("port-version", 0) != revision:
        problems.append(
            f"versions/baseline.json: {name} defaults to "
            f"{entry.get('baseline')}#{entry.get('port-version', 0)}, but "
            f"ports/{name}/vcpkg.json declares {version}#{revision}",
        )

    return problems


def run(root: pathlib.Path, *, how: bool = False) -> int:
    ...
    if git("status", "--porcelain", "--", "ports", repo=root):
        problems.append(
